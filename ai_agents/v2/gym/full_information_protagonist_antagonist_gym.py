...
GOAL_LINE_Y = 50.0              # center of goal line along +y / -y
...
# Goal line position (along y) and width (along x)        # where the goal line is in y
GOAL_HALF_WIDTH = 20.0      # posts at x ∈ [-10, +10]  (tune this)
...
TABLE_OUT_X = 70.0                # soft bounds; tune to your table size
TABLE_OUT_Y = 65.0
...
class FoosballEnv(MujocoTableRenderMixin, gym.Env):
    ...
    def __init__(self, antagonist_model=None, play_until_goal=False,
                 verbose_mode=False, debug_free_ball=False, render_mode="human",kick_drill_mode=False):
        ...
        self.kick_drill_mode = kick_drill_mode
        xml_file = SIM_PATH
    ...
    def reset(self, *, seed=None, options=None):
        ...
        if self.kick_drill_mode:
            # Deterministic “ball at foot” drill
            self._spawn_ball_in_front_of_attack_guy(margin=0.02)
        else:
            # --- Your existing random serve ---
            x_min, x_max = -25.0, 25.0
            y_min, y_max = -10.0, 10.0

            spawn_x = self.np_random.uniform(x_min, x_max)
            spawn_y = self.np_random.uniform(y_min, y_max)

            self.data.qpos[self.ball_x_qpos_adr] = spawn_x
            self.data.qpos[self.ball_y_qpos_adr] = spawn_y

            vx = self.np_random.uniform(-0.5, 0.5)
            base_speed = self.np_random.uniform(1.0, 2.5)
            vy = base_speed * self._direction_sign_for_protagonist

            self.data.qvel[self.ball_x_qvel_adr] = vx
            self.data.qvel[self.ball_y_qvel_adr] = vy

            mujoco.mj_forward(self.model, self.data)

        # --- book-keeping exactly as you already have ---
        self.simulation_time = 0.0
        self.no_progress_steps = 0
        self.ball_stopped_count = 0
        self._debug_step_counter = 0
    ...
    def step(self, protagonist_action):
        ...
        if speed > MAX_BALL_SPEED:
            scale = MAX_BALL_SPEED / (speed + 1e-8)
            self.data.qvel[self.ball_x_qvel_adr] *= scale
            self.data.qvel[self.ball_y_qvel_adr] *= scale
            # re-forward to keep derived quantities consistent
            mujoco.mj_forward(self.model, self.data)
            if self.verbose_mode:
                print(f"[BALL CLAMP] speed {speed:.2f} -> clamped to {MAX_BALL_SPEED}")
        ...
        info.update(getattr(self, "_last_termination_info", {}))
    ...
    # -------------------------------------------------------------------------
    def _spawn_ball_in_front_of_attack_guy(self, margin=0.05):
        """
        Place the planar ball (ball_x, ball_y) just in front of y_attack_guy2,
        with no initial velocity, and resolve any tiny overlaps with players.
        Used to create a guaranteed 'kickable' state.
        """
        import mujoco
        ...
        if attack_guy_geom_id < 0:
            if self.verbose_mode:
                print("[KICK SPAWN] y_attack_guy2 not found, skipping special spawn")
            return
        ...
        # Ball radius from ball_phys geom
        ball_radius = float(self.model.geom_size[self.ball_geom_id][0])
        # Player effective radius from bounding radius
        guy_radius = float(self.model.geom_rbound[attack_guy_geom_id])

        safe_offset = ball_radius + guy_radius + margin

        # Use protagonist direction sign to decide which side is "in front"
        # If you always attack +y, this is +1.0
        forward_sign = float(self._direction_sign_for_protagonist)

        # Set ball planar qpos to be in front of the guy in (x,y)
        self.data.qpos[self.ball_x_qpos_adr] = guy_xyz[0]
        self.data.qpos[self.ball_y_qpos_adr] = guy_xyz[1] + forward_sign * safe_offset
        ...
        # --- Resolve any remaining ball–player overlap nicely ---
        def ball_in_contact_with_player(model, data):
            for i in range(data.ncon):
                c = data.contact[i]
                g1, g2 = c.geom1, c.geom2
        ...
        while True:
            ...
            penetration = -float(c.dist)  # > 0 if overlapping
            if penetration <= 0:
                break

            n = np.array(c.frame[:3], dtype=float)  # contact normal (world)
            shift = penetration + 1e-3

            # Move ONLY in the planar directions
            self.data.qpos[self.ball_x_qpos_adr] += shift * n[0]
            self.data.qpos[self.ball_y_qpos_adr] += shift * n[1]
            ...
            if tries >= max_tries:
                if self.verbose_mode:
                    print("[KICK SPAWN] couldn't fully resolve ball-player overlap")
                break

        if self.verbose_mode:
            ball_pos, ball_vel = self._get_ball_obs()
            print(
                "[KICK SPAWN] ball in front of attack guy:",
                f"pos=({ball_pos[0]:.3f}, {ball_pos[1]:.3f}), vel={ball_vel}"
            )
    ...
    def terminated(self):
        ...
        ball_stagnant = self.ball_stopped_count >= STAGNANT_STEPS
        out_of_play   = (abs(ball_x) > TABLE_OUT_X) or (abs(ball_y) > TABLE_OUT_Y)
        max_episode_seconds = 10.0
        over_max_time = self.simulation_time >= max_episode_seconds

        if self.play_until_goal:
            terminated = unhealthy or goal_scored or out_of_play
        else:
            terminated = (
                unhealthy
                or goal_scored
        ...
        self._last_termination_info = {
            ...
            "ball_stagnant": bool(ball_stagnant),
            "out_of_play": bool(out_of_play),
            "over_max_time": bool(over_max_time),
        }
        ...
        if self.verbose_mode and terminated:
            ...
            print(
                ...
                f"Ball stagnant: {ball_stagnant}, Out of play: {out_of_play}, "
                f"Over max time: {over_max_time}"
            )
            print("x: ", ball_x, "y: ", ball_y)
