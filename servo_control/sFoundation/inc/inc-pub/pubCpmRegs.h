...
#define SC_USER_DESCR_SZ        (SC_USER_DESCR_SPAN*SC_USER_DESCR_CHUNK)
                                                    /** \endcond **/
...
// Motion Status Group
	...
	CPM_P_MOVE_OPTIONS,                  // 98 62 Extended move option specificatations

    CPM_P_INA_TC = 99,                   // 99  63 Input A filter (ms)
    CPM_P_INB_TC = 102,                  // 102 66 Input B filter (ms)
...
// Factory Settings
    ...
    CPM_P_DRV_FACT_FS_BUSV,              // 266 0a Full scale bus volts
...
// Motor Parameters                     
    ...
    CPM_P_DRV_MTR_OHMS=313,              // 313 39 Motor: Resistance (ohm)
...
struct _cpmHwConfigFlds {
    ...
    typedef enum _motorTypes {
        ...
        MOTOR_STEPPER,                  ///<  1: Stepper motor
        MOTOR_BRUSH_ROTARY,             ///<  2: Rotary, brush
    } motorTypes;                       ///< Motor Types

	/// Homing styles for the HomingStyle field.
	typedef enum _homingStyles {
		HOME_TO_SWITCH = 0,			///<  0 Homing to switch input
		HOME_TO_HARDSTOP = 1		///<  1 Homing to a hardstop
	} homingStyles;

    /**
     * Field interpretation for the extended homing features.
     **/
    typedef enum _homingExtended {
        /*!
         * Legacy mode, use the settings in HomingStyle field.
         */
        USE_LEGACY,
        /*!
            \brief <i>Home to Shaft Position</i>

            Homing target is the specified mechanical shaft position.
        **/
        HOME_TO_SHAFT_POSN,
    } homingExtended;
    ...
    **/
    ...
    /*!
        \brief <i>Bit enabling the d current heater</i>

        If true, d current heater will be turned on.
    **/
    BFld EnableDHeater : 1;                     // 9
    /**
    ...
    **/
    homingStyles HomingStyle : 1;               // 14

    /*!
        \brief <i>Step & Direction Command Input Format</i>

        If asserted, the input format will use the quadrature format.
        If not asserted, the input format is step & direction.
    ...
    **/
     ...
     *
     *  Set to 0 for all 3-phase powered motors and 1 for single phase.
     *
...
#if defined _MICROLOOP
    /*!
        \brief Index Available

        If true, the encoder has a once per turn index pulse.
    **/
    unsigned IndexAvailable: 1;                 // 18
#else
                                                    /**  INTERNAL_DOC **/
    BFld: 1;                                    // 18
                                                    /**  **/
    ...
    /*!
        \brief <i>Bit enabling the encoder heater</i>

        If true, the encoder heater will be turned on.
    **/
    unsigned EnableEncHeater : 1;               // 19
    ...
    **/
    ...
    /*!
     *     \brief <i>Extended Homing Modes</i>
     *
     *     This field augments the HomingStyle field.
     *
     */
    homingExtended HomingExtended : 3;          // 24-26

#if defined _MICROLOOP

    /*!
        \brief Index Vector Refinement

        If true, the electrical angle is synced to the
        Reference Offset (RO) value when an index pulse is seen.
    **/
    unsigned IndexVectorRefinement: 1;          // 27

    /*!
        \brief Encoder Direction Swap

        If true, the encoder counts in the opposite direction.
    **/
    unsigned EncoderDirSwap : 1;                // 28
    /*!
        \brief Motor Phase Direction Swap

        This field will change the direction the motor shaft moves
        for a when torque is applied by changing applied phase voltages.
    **/
    unsigned PhaseDirSwap : 1;                  // 29
    ...
    /**
     * \brief Input Filter Low Speed
     *
     *  If set, use low speed input filtering.
     *  If clear, use normal input filtering.
     *
     *    \note This feature was introduced with firmware version 2.0.0
     **/
    unsigned InFilterLowSpeed     : 1;          // 31
};
...
struct PACK_BYTES _cpmAppConfigFlds {
    ...
    **/
    ...
    /**
     * \brief <i>Disable Precision Homing</i>
     *
     * Turns off the feature that uses the index pulse locations to
     * account for the home hardstop/input locations being inconsistent.
     **/
    BFld DisablePrecisionHoming : 1;           // 15
    ...
    **/
    ...
    /*!
        \brief <i>Enable Vector Refinement</i>

        If set, this allows the use of the Vector Refinement tool.

        \note Used by ClearView
     **/
    BFld UseVectorRefinement : 1;               // 17

#if defined(_MICROLOOP)
    /*!
        \brief <i>Check 5V Load</i>

        If set, check the 5V load to detect a disconnected motor encoder.
        When a disconnected encoder is detected, the drive will reinitialize
        as necessary when the encoder is reconnected and the alert is cleared.

     **/
    BFld Check5VLoad : 1;                       // 18
#else
                                                    /**  INTERNAL_DOC **/
    BFld: 1;                                    // 18
...
/**
    ...
    This page provides the detailed list of fields in the ClearPath SC status
    register.  
    The lower 32 bits of this register are attentionable.

    \see sFnd::ValueStatus for information on accessing different status
    registers within the node.
...
struct PACK_BYTES _cpmStatusRegFlds {
    ...
    **/
    ...
    /**
        ...
        This field is set when the bit-wise AND of the
        [User Alert register](@ref sFnd::IAttnNode::AlertMask)
    ...
    /**
        ...
        If this field is true, a node is not ready to execute motion.
        You must make sure to [clear shutdowns](@ref sFnd::IStatus::AlertsClear),
        ensure the node is [enabled](@ref sFnd::INode::EnableReq),
        and [clear any nodestops](@ref sFnd::IMotion::NodeStopClear).
    ...
    **/
    ...
    /**
        ...
        This field indicates the Node can receive additional move commands which
        will be stored in the Node's move buffer.
	...
	**/
	...
	/**
		\brief Attention-0 state.

		This field asserts when the output register Attention-0 bit is asserted.
	...
	**/
	...
	/**
        ...
        When this field is asserted, the Node is unable to execute moves in the
        positive direction. The input configured through Clearview to limit
        motion in the positive direction is currently limiting motion.
    ...
    **/
    ...
    /**
        ...
        When this field is asserted, the Node is unable to execute moves in the
        negative direction. The input configured through Clearview to limit
        motion in the negative direction is currently limiting motion.
    ...
    **/
    ...
    /**
        ...
        \see [CPM NodeStops Page](@ref CPMnodeStopPage)
    ...
    **/
    ...
    /**
        ...
        Indicates that a homing sequence has been executed.
    ...
    **/
    ...
    /**
        ...
        Indicates that a homing sequence is being executed.    
    ...
    **/
    ...
    /**
        ...
        Indicates that the drive is about to go disabled.
    ...
    **/
    ...
    /**
        ...
        This field asserts when a move is complete. If the axis goes out-of-range
        this field will reset until we settle again.
   ...
   **/
    ...
    /**
        ...
        This field indicates that the node is currently not tracking within the
        In-Range window. This is especially common during moves.

        \see \ref CPMmoveDonePage for more information regarding setting up the
        In-Range window in ClearView
    ...
    **/
    ...
    /**
        ...
        This field asserts when the move is
        [B From End](@ref sFnd::IMotionAdv::BeforeEndDistance) of the move.
    ...
    **/
    ...
    /**
        ...
        the  Absolute Position location specified in the torque limiting dialog
        window of ClearView.
    ...
    **/
    BFld InvInB                      : 1;    // 25
    /**
        \brief <i>Latched Velocity Move completed and settled</i>.

        This field is similar to the
        [Move Done](@ref cpmStatusRegFlds::MoveDone)
        field of this register. Once asserted, it will remain so even if the
        motor velocity is jostled out of range.
        ...
        autonomous handling.
    **/
    BFld AtVelLatched                : 1;    // 26
    /**
        \brief <i>Latched Move completed and settled</i>.

        This field is similar to the
        [Move Done](@ref cpmStatusRegFlds::MoveDone)
        field of this register. Once asserted, it will remain so even if the
        motor is jostled out of range.
        ...
        autonomous handling.
    **/
    BFld MoveDoneLatched             : 1;    // 27
    /**
        \brief <i>A From Start</i> of move.

        This field asserts when the move is
        [A From Start](@ref sFnd::IMotionAdv::AfterStartDistance) of the move.
    ...
    **/
    ...
    /**
        ...
        disabled state. [Clear nodestops](@ref sFnd::IMotion::NodeStopClear)
        to return the node to its previous enable state.
    ...
    **/
    BFld InDisableStop               : 1;    // 34
                                            /** \cond INTERNAL_DOC **/
    /**
        \brief Controlled Stop in effect.
    **/
    ...
    **/
    ...
    /**
        ...
        This field is asserted when the vector has not been finalized.  
    ...
    **/
    ...
    /**
        ...
        The move is considered totally complete when the
        [Move Done](@ref _cpmStatusRegFlds::MoveDone) has been asserted.
    **/
    BFld MoveCmdComplete             : 1;    // 38
    /**
        \brief In hard-stop mode.

        This field asserts when the ClearPath-SC has entered the <i>Hard Stopped</i>
        mode during homing and the node is applying clamping torque. 

        \see [Homing Page](@ref HomingPage)
    **/
    BFld InHardStop                  : 1;    // 39 
    /**
        \brief Shutdown state.

        These fields describe the shutdown condition of the node:
    ...
    **/
    ...
    /**
        \brief Step and direction inputs are active.

        This field is asserted when the step and direction inputs are active.
    **/
    ...
    **/
    ...
    /**
        \brief Vector is being refined.

        Indicates when the vector refinement process is in progress.
    **/
    BFld RefiningVector              : 1;   // 47
                                           /** \endcond  **/
...
/**
    ...
    There are accumulating types that clear upon reading and one snapshot
    ...
    similar to the sFnd::IStatus:Rise, but currently asserted status is not
...
/**
    ...
    This page provides the detailed list of the Clearpath SC alerts.
    These fields describe the reason a shutdown has occurred, and
    potential solutions.
...
struct PACK_BYTES _cpmAlertFlds {
    ...
    **/
    ...
    The move was canceled due to the
    [Velocity Limit](@ref sFnd::IMotion::VelLimit) of the attempted move
    exceeding the encoder's or RAS/G-Stop's maximum allowed speed.
    ...
    **/
    ...
    is activated. Motion in the direction of the limit is prevented.
    ...
    **/
    ...
    Remedies:
    - Decrease the distance of the commanded move such that it ends within
    the bounds of the current soft limit settings.
    ...
    **/
    ...
    The motor has detected an excessively low operating
    temperature. This condition is recoverable when the
    temperature has been sufficiently increased.
    ...
    **/
    ...
    has qualified for the hardstop condition, the mechanics must stay reasonably
    still while motor torque is applied at the foldback level.
    If the coupling or hardstop should break free, the axis will move in the
    direction of the hardstop and this alert will be issued.

    Remedies:
    - Checking homing parameters in ClearView
    - Ensure that the hardstop for this axis is solid and unmovable.
    - For High friction applications, or machines subject to binding issues,
    ensure that the hardstop entry torque defined through ClearView
    is high enough that the motor will not qualify for the hardstop condition
    except when the axes is against the hardstop on the machine.
    **/
    ...
    The position tracking error has exceeded the user specified critical
    tracking limit. This is a resettable shutdown condition.
    ...
    **/
    ...
    The RMS torque of the motor has exceeded the critical limit. This is a
    resettable shutdown condition.

    Remedies:
    - Check the mechanics for abnormally high friction/binding.
    - Reduce velocity, acceleration, and/or duty cycle.
    - Check that the configuration file is appropriate for this motor.
    - If the motor is actively cooled with a fan raise the continuous current
    limit of the motor appropriately.
    - Use a larger motor.
    **/
    ...
    **/
    ...
    This warning occurs when full torque was commanded to drive the motor.
    ...
    **/
    ...
    This is recoverable when the bus voltage returns to the operating range of the motor.
    ...
    **/
    ...
    /**
    \brief Create a newline-delimited string of the alerts set.

    Create a newline-delimited string of the alerts set.
    ...
    \return The original pointer to the parameter \a buffer. This allows this 
    function to be the argument of a printing statement or other formatting 
    function.
    **/
    MN_EXPORT char * MN_DECL StateStr(char *buffer, size_t size) const;
                                                    /** \endcond  **/
...
/**
    ...
    If the <i>Status Register's</i>
    [HwFailure](@ref _cpmStatusRegFlds::HwFailure) bit is asserted, this is a
    ...
    Many shutdowns can be cleared via the sFnd::IStatus::AlertsClear function
    to allow mcontinued node operations. Most fatal errors cannot be cleared
    by this mechanism and require a power-cycle or node restart to clear them.
...
union PACK_BYTES _cpmTuneConfigReg {
    ...
    /**
        * Configuration State
        *
        * Maintained by the host, this set of enumerations is used to insure
        * the user has loaded a configuration file appropriate for their
        * mechanics. The loader will check for tuned state provenance in this
        * register and set to legacy if incoming tuning is set to FACTORY_TUNED.
    **/
    typedef enum _configStates {
        FACTORY_TUNED,                      ///< 0 Unloaded shaft default tuning
        AUTO_TUNED,                         ///< 1 Auto-tuned
        MANUAL_TUNED,                       ///< 2 Manually tuned
        LEGACY_TUNED                        ///< 3 Config loaded without provenance
    } configStates;
    ...
    struct _cpmTuneConfigFlds {
       ...
       **/
        ...
        /**
            ...
            Apply fine tuning scale factors to Kp/Ki and set the target window 
            size.
        ...
        **/
        ...
        BFld :3;                               // 17-19
        /**
             \brief Configuration File Source

             Used by the host, this field records the provenance of the
             configuration file.
        **/
        configStates ConfigFileSource : 3;     // 20-22                                                    
    };                               
    /**
...
struct _cpmOptionRegFlds {
    ...
    *                    -----------  ----------------------------------- */
    ...
    BFld ShipVerSet    : 1;     // 20: Shipout version has been set
