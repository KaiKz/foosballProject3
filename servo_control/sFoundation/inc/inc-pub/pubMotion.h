...
//
// COPYRIGHT NOTICE:
//      (C)Copyright 2008-2021 Teknic, Inc.  All rights reserved.
//
...
typedef enum _mgPosnStyles {
    ...
    MG_MOVE_STYLE_DWELL_OPTION = 0x04,    ///< Create post move dwell (newer f/w)
                                        /** \cond INTERNAL_DOC **/
    MG_MOVE_STYLE_IMMEDIATE_OPTION = 0x100 ///< Start execution immediately (newer f/w, internal)
                                        /** \endcond **/
} mgPosnStyles;
    ...
    struct {
        ...
        **/
        ...
        unsigned    reciprocate : 1;    ///< Reciprocating moves/repeat
                                        /** \cond INTERNAL_DOC **/
        unsigned    immediate   : 1;    ///< Start executing immediately
                                        /** \endcond **/
    } fld;                              ///< Field access to move styles.
#ifdef __cplusplus
    /// Useful converter to convert integer code to style container.
    _mgPosnStyle(mgPosnStyles how) {
        styleCode = how;
    }
    _mgPosnStyle() {
        styleCode = MG_MOVE_STYLE_NORM;
    }
    void operator=(const mgPosnStyles &how) {
        styleCode = how;
    }
#endif
} mgPosnStyle;
...
typedef enum _mgVelStyles {
    ...
    **/
    ...
    /**
     * Regular velocity that starts immediately
     */
    MG_MOVE_VEL_IMMEDIATE = 0x10
} mgVelStyles;
...
typedef union _mgVelStyle {
    ...
    struct {
        ...
        **/
         ...
         *  Immediate move
         *
         *  This field tells the drive not to wait for the current move to
         *  complete before starting this move.
         */
        unsigned immediate : 1;
                                                        ...
                                                        /** \cond INTERNAL_DOC **/
#ifdef __cplusplus
    _mgVelStyle(mgVelStyles how) {
        styleCode = how;
    }
    _mgVelStyle() {
        styleCode = MG_MOVE_VEL_STYLE;
    }
    void operator=(const mgVelStyles &how) {
        styleCode = how;
    }
#endif
} mgVelStyle;
...
enum _nodeStopCodes {
    ...
    **/
    STOP_TYPE_RAMP = 0x01,          // Kill Motion with decel limit
                                                /** \cond ISC_DOC **/
    /*!
        Stop motions after current motion cycle finishes execution
        For use in reciprocal, and repeat moves
    **/
   STOP_TYPE_AFTER_CYCLE = 0x02,        // Kill Motion end of period
    /*!
