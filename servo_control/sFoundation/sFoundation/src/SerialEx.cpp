...
#if TRACE_THREAD || TRACE_DESTRUCT
    _RPT2(_CRT_WARN,
          "%.1f CSerialEx (destruction started), id=" THREAD_RADIX "\n",
...
#if (defined(_WIN32)||defined(_WIN64))
    CSerial::SERAPI_ERR lLastError = CSerial::Open(portNum, READ_BUF_LEN,
                                                   64, true);
#else
...
bool CSerialEx::TestAndPushHiPacket(char nextChar) {
    ...
    if (inHighPrioState()  // Any "restart" in high-prio is bad
        || (!MN_PKT_IS_HIGH_PRIO(parser->PktType)
...
#define PKT_OVERHEAD_LEN (MN_API_PACKET_HDR_LEN+MN_API_PACKET_TAIL_LEN)
            ...
            if (m_lowPrioPktIndx
                >= m_lowInProcPacket.Fld.PktLen + PKT_OVERHEAD_LEN) {
                // Non-zero means corruption
                if (m_lowChecksum & 0x7F) {
                    // Checksum Error!
                    SendErrToApp(ND_ERRNET_CHKSUM,
                                 m_lowInProcPacket.Fld.Addr, 0);
                }
                else {
        ...
        case READ_STATE_HP_PAYLOAD:
            ...
            // Have we reached the checksum?
            if (m_hiPrioPktIndx
                >= m_hiInProcPacket.Fld.PktLen + PKT_OVERHEAD_LEN) {
                // Non-zero means corruption
                if (m_hiChecksum & 0x7F) {
                    // Checksum Error!
                    SendErrToApp(ND_ERRNET_CHKSUM,
                                 m_hiInProcPacket.Fld.Addr, 0);
                }
                else {
                    // Setup the character length
        ...
        default:
            _RPT1(_CRT_ASSERT,
                  "CSerialEx::ProcessNextChar unknown state %d\n", m_state);
            PacketParseReset();
            // Process the low priority packet
            break;
    }
...
int CSerialEx::Run(void *context) {
...
#if TRACE_THREAD || TRACE_DESTRUCT
    _RPT2(_CRT_WARN,
          "%.1f CSerialEx::Run thread id=" THREAD_RADIX " starting\n",
...
#if TRACE_THREAD || TRACE_DESTRUCT
    _RPT2(_CRT_WARN,
          "%.1f CSerialEx::Run thread id=" THREAD_RADIX " looping\n",
          infcCoreTime(), CurrentThreadID());
#endif
    ...
    do {
        ...
        if (m_packetMode) {
            ...
            while (m_rdBuffer.getChar(thisChar)
        ...
        // Some port error was detected, log each one, if not break!
            ...
            if (eError & (CSerial::EErrorRxOver | CSerial::EErrorOverrun)) {
                ...
                _RPT1(_CRT_WARN, "%.1f CSerialEx::RX overrun\n",
                      infcCoreTime());
                Purge();
                eError = (CSerial::EError)(eError & ~(CSerial::EErrorRxOver |
                                                      CSerial::EErrorOverrun));
            }
            if (eError & (CSerial::EErrorBreak | CSerial::EErrorFrame)) {
                ...
                if (!m_breakSent && !m_rdAutoFlush) {
                    ...
                    _RPT1(_CRT_WARN,
                          "%.1f CSerialEx::Framing Error(expected)\n",
                          infcCoreTime());
                }
#endif
                ...
                // Reset filter
                m_breakSent = false;
                eError = (CSerial::EError)(eError & ~(CSerial::EErrorBreak |
                                                      CSerial::EErrorFrame));
            }
            if (eError) {
                // Skip the rest
...
//
//  SYNOPSIS:
void CSerialEx::SendErrToApp(mnNetErrs errorType, unsigned addr,
                             unsigned data) {
...
#if TRACE_ERR
    _RPT3(_CRT_WARN,
          "CSerialEx::SendErrToApp: errorType = %d ; addr = %d ; data = %d\n",
          errorType, addr, data);
#endif
}
//                                                                            *
...
void CSerialEx::SetMode(CSerialMode theNewMode) {
    switch (theNewMode) {
        ...
        default:
            _RPTF1(_CRT_ASSERT,
                   "CSerialEx::SetMode unsupported type %d\n", theNewMode);
            break;
    }
}
...
void CSerialEx::convert8to7(packetbuf &inBuf, packetbuf &outBuf) {
    ...
    while (num8 > 0) {
        ...
        switch (outBytes) {
            case 8:
                d[7] = 0x7f & ((unsigned char)s[6] >> 1);
            // no break OK (special line for Code Analyzer to suppress warning)
            case 7:
                d[6] = 0x7f & ((s[6] << 6) | ((unsigned char)s[5] >> 2));
            // no break OK (special line for Code Analyzer to suppress warning)
            case 6:
                d[5] = 0x7f & ((s[5] << 5) | ((unsigned char)s[4] >> 3));
            // no break OK (special line for Code Analyzer to suppress warning)
            case 5:
                d[4] = 0x7f & ((s[4] << 4) | ((unsigned char)s[3] >> 4));
            // no break OK (special line for Code Analyzer to suppress warning)
            case 4:
                d[3] = 0x7f & ((s[3] << 3) | ((unsigned char)s[2] >> 5));
            // no break OK (special line for Code Analyzer to suppress warning)
            case 3:
                d[2] = 0x7f & ((s[2] << 2) | ((unsigned char)s[1] >> 6));
            // no break OK (special line for Code Analyzer to suppress warning)
            case 2:
                d[1] = 0x7f & ((s[1] << 1) | ((unsigned char)s[0] >> 7));
            // no break OK (special line for Code Analyzer to suppress warning)
            case 1:
                d[0] = 0x7f &   s[0];
                // no break OK (special line for Code Analyzer to suppress warning)
...
#if TRACE_DESTRUCT||TRACE_THREAD
    _RPT2(_CRT_WARN,
          "%.1f CSerialEvt::Run restarted, id=" THREAD_RADIX "...\n",
          infcCoreTime(), ThreadID());
#endif
    while (!Terminating()) {
        ...
        if ((lastError = pPort->CommEventWaitInitiate()) != 0) {
            ...
            if (pPort->IsOpen() && !Terminating()) {
                _RPT1(_CRT_WARN,
                      "CSerialEvt::Run CommEventWaitInitiate failed,err=%d\n",
                      lastError);
            }
#endif
            break;
