    ...
    mnNetInvRecords *initInfo)  // Ptr to information return
{
    ...
    switch (pPortSpec->PortType) {
        ...
        default:
            _RPT3(_CRT_WARN,
                  "%.1f mnInitializeProc(%d) no port class for this type %d\n",
    ...
    if (startErr == MN_OK) {
        ...
        if (resetNodes && initErr == MN_OK) {
            ...
            if (resetErr != MN_OK) {
                infcErrInfo errInfo;
                _RPT3(_CRT_WARN,
                      "%.1f mnInitializeProc(%d) reset failed code 0x%X\n",
        ...
        // and will talk.
        if (startErr >= MN_ERR_CMD_INTERNAL &&
            startErr <= MN_ERR_CMD_ERR_END) {
            startErr = MN_OK;
        }
...
MN_EXPORT cnErrCode MN_DECL mnInitializeSystem(
    ...
    for (cNum = 0; cNum < NET_CONTROLLER_MAX; cNum++) {
        ...
        if (theErr != MN_OK) {
            _RPT3(_CRT_WARN,
                  "%.1f mnInitializeSystem stopping error @%d, code=0x%0X\n",
    ...
    for (netaddr i = 0 ; i < netCount ; i++) {
        ...
        extern infcNetStateCallback userNetStateFunc;
        if (userNetStateFunc != NULL) {
            (*userNetStateFunc)(i,
